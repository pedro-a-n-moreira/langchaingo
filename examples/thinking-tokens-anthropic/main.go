...
Consider trade-offs between consistency, availability, and partition tolerance.`),
	...
	// Example 6: Combined with prompt caching
	fmt.Println("\nExample 6: Thinking + Prompt Caching")
	fmt.Println("-------------------------------------")
	
	// Large context that benefits from caching
	largeContext := `You are an expert system architect with deep knowledge of:
...
Consider all these aspects when answering questions.`

	messages6 := []llms.MessageContent{
		{
		...
		},
		{
			Role: llms.ChatMessageTypeHuman,
			Parts: []llms.ContentPart{
				llms.TextPart("How would you migrate a monolithic application to microservices?"),
			},
		},
	}

	resp6, err := llm.GenerateContent(ctx, messages6,
		llms.WithMaxTokens(1000),
		llms.WithThinkingMode(llms.ThinkingModeHigh),
		llms.WithAnthropicCachingHeaders(), // Enable caching beta
	)
	if err != nil {
		log.Printf("Error: %v", err)
		return
	...
	fmt.Printf("Response: %s\n", resp6.Choices[0].Content)
	printTokenUsage(resp6.Choices[0].GenerationInfo)
...
func printTokenUsage(generationInfo map[string]any) {
	...
	// Cache tokens (if using caching)
	if cacheCreation, ok := generationInfo["CacheCreationInputTokens"].(int); ok && cacheCreation > 0 {
		fmt.Printf("Cache Creation Tokens: %d\n", cacheCreation)
	}
	
	if cacheRead, ok := generationInfo["CacheReadInputTokens"].(int); ok && cacheRead > 0 {
